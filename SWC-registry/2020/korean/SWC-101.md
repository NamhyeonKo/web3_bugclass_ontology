...
오버플로우/언더플로우는 산술 연산이 타입의 최대값 또는 최소값에 도달할 때 발생합니다. 예를 들어 숫자가 uint8 타입에 저장되면, 이는 0에서 2^8-1까지의 범위를 가진 8비트 부호 없는 숫자로 저장됨을 의미합니다. 컴퓨터 프로그래밍에서 정수 오버플로우는 산술 연산이 주어진 비트 수로 표현할 수 있는 범위를 벗어나는 숫자 값을 생성하려고 할 때 발생합니다 - 최대값보다 크거나 표현 가능한 최소값보다 작은 값입니다.

## 해결방법

스마트 컨트랙트 시스템 전반에 걸쳐 산술 연산에 대해 검증된 안전한 수학 라이브러리를 일관되게 사용하는 것이 권장됩니다.

...
Solidity는 다중 상속을 지원합니다. 즉, 하나의 컨트랙트가 여러 컨트랙트를 상속할 수 있습니다. 다중 상속은 [다이아몬드 문제](https://en.wikipedia.org/wiki/Multiple_inheritance#The_diamond_problem)라는 모호성을 도입합니다: 두 개 이상의 기본 컨트랙트가 동일한 함수를 정의하는 경우, 자식 컨트랙트에서 어떤 함수를 호출해야 할까요? Solidity는 역방향 [C3 선형화](https://en.wikipedia.org/wiki/C3_linearization)를 사용하여 이 모호성을 처리하며, 이는 기본 컨트랙트 간의 우선순위를 설정합니다.

이러한 방식으로 기본 컨트랙트는 서로 다른 우선순위를 가지므로 상속 순서가 중요합니다. 상속 순서를 무시하면 예기치 않은 동작이 발생할 수 있습니다.

## 해결방법

여러 컨트랙트를 상속할 때, 특히 동일한 함수를 가진 경우, 개발자는 올바른 순서로 상속을 신중하게 지정해야 합니다. 일반적인 규칙은 더 /일반적인/ 것에서 더 /구체적인/ 것으로 컨트랙트를 상속하는 것입니다.
